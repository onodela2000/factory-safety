<template>
  <div class="result-container">
    ...
    <div v-if="incorrectQuizzes.length > 0" class="incorrect-quizzes">
      <h3>間違えた問題の解説:</h3>
      <div
        v-for="quiz in incorrectQuizzes"
        :key="quiz.question"
        class="incorrect-quiz"
      >
        <h4>{{ quiz.question }}</h4>
        <p class="answer">正解: {{ quiz.options[quiz.correctAnswer] }}</p>
        <p class="answer">あなたの回答: {{ quiz.options[quiz.userAnswer] }}</p>
        <p v-if="quiz.explanation" class="explanation">
          解説: {{ quiz.explanation }}
        </p>
      </div>
    </div>
    <div style="display: flex; flex-direction: column; align-items: center;">
      <van-button type="success" @click="goToSurvey" class="survey-button" style="width: 300px;">
        アンケートに答える
      </van-button>
      <van-button type="primary" @click="goToHome" class="home-button" style="width: 300px;">
        ホームに戻る
      </van-button>
    </div>
...
const correctPercentage = computed(() => {
  ...
  const correctAnswers = quizResults.value.filter(
...
const goToSurvey = () => {
  window.open("https://forms.gle/TGon95H1FN5AdvgL8", "_blank");
};
...
const confettiStyle = (n) => {
  ...
  return {
    ...
    "--speed": `${5 + rand() * 10}s`,
    "--delay": `${-1 - rand() * 2}s`,
    "--color": `hsl(${210 + rand() * 50}, ${70 + rand() * 30}%, ${
      70 + rand() * 30
    }%)`,
  };
};
